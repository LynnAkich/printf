...
int write_num(int ind, char buffer[], int flags, int width, int prec, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS && padd == ' ') /* Assign extra char to left of buffer */
		{
			if (extrs_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[ind], length) + write(1, &buffer[ind], a - 1));
		}
		...
		else if (!(flags & F_MINUS) && padd == '0') /*extra char to left of padd */
		{
			...
			return (write(1, &buffer[padd_start], a - padd_start) + write(1, &buffer[ind], length - (1 - padd_start)));
		}
	}
	if (extra_c)
		buffer[--ind] = extra_c;
 ...
 * @ind: Index at which number starts
...
{
	/* Number is strored at buffer's right and starts at position i */
...
int write_pointer(char buffer[], int ind, int length, int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0') /* extra char to left of padd */
		{
			...
			buffer[1] = '0';
			buffer[2] = 'x';
			return (write(1, &buffer[padd_start], a - padd_start) + write (1, &buffer[ind], length - (1 - padd_start) - 2));
		}
	}
	buffer[--ind] = 'x';
	buffer[--ind] = '0';
